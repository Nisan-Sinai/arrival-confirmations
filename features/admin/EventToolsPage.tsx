...
export async function EventToolsPage({
  params,
  locale,
}: {
  ...
  const copy = getAppCopy(locale).toolsPage;
  ...
  const inviteUrl = `${await resolveRequestOrigin()}${localePath(locale, `/e/${event.public_id}`)}`;
      ...
      <Container width="wide">
        ...
        <div className="mt-4">
          <p className="text-eyebrow text-accent-strong font-semibold">
            {isPro ? copy.proEyebrow : copy.premiumEyebrow}
          </p>
          <h1 className="text-h1 text-primary mt-2 font-bold">
            {copy.title} · {event.title}
          </h1>
          <p className="text-muted-foreground mt-3 max-w-3xl leading-relaxed">
            {isPro ? copy.proIntro : copy.premiumIntro}
          </p>
        </div>

        {!enabled ? (
          <Card padding="lg" className="mt-8">
            <h2 className="text-h2 text-primary font-bold">{copy.lockedTitle}</h2>
            <p className="text-muted-foreground mt-3 leading-relaxed">{copy.lockedBody}</p>
            <Link href={localePath(locale, '/pricing')} className={`${buttonClass({ size: 'lg' })} mt-6`}>
              {copy.plans}
            </Link>
          </Card>
        ) : (
